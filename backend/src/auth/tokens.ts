...
export const verifyAccessToken=(token:string)=>jwt.verify(token,secret(),{issuer:"pg-router-ai",audience:"pg-router-ai-api"}) as jwt.JwtPayload;
export async function issueRefresh(userId:string,familyId=randomUUID()){const raw=randomBytes(48).toString("base64url"),id=randomUUID();await authRepository.saveRefresh({id,userId,familyId,tokenHash:hashToken(raw),expiresAt:Date.now()+refreshMs,revokedAt:null,replacedBy:null});return {raw,id,familyId};}
