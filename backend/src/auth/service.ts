...
import { issueRefresh, rotateRefresh, signAccessToken } from "./tokens";
...
async function createOtp(email:string,purpose:OtpPurpose,payload:Record<string,unknown>){const id=randomUUID(),code=String(randomInt(100000,1000000));await authRepository.saveOtp({id,email,purpose,codeHash:hashCode(id,code),payload,attempts:0,expiresAt:Date.now()+600000,consumedAt:null});await queueOtpEmail(email,code,purpose);}
async function consumeOtp(email:string,code:string,purpose:OtpPurpose){const otp=await authRepository.getLatestOtp(normalize(email),purpose);if(!otp||otp.consumedAt||otp.expiresAt<Date.now()||otp.attempts>=5)throw new Error("Invalid or expired code");otp.attempts+=1;if(hashCode(otp.id,code)!==otp.codeHash){await authRepository.updateOtp(otp);throw new Error("Invalid or expired code");}otp.consumedAt=Date.now();await authRepository.updateOtp(otp);return otp;}
...
export async function login(email:string,password:string){const user=await authRepository.findUserByEmail(normalize(email));if(!user||!await bcrypt.compare(password,user.passwordHash))throw new Error("Invalid email or password");return createSession(user);}
export async function refresh(raw:string){const rotated=await rotateRefresh(raw),user=await authRepository.findUserById(rotated.userId);if(!user)throw new Error("Account not found");return {accessToken:signAccessToken(user.id,user.role),refreshToken:rotated.raw,user:safeUser(user)};}
