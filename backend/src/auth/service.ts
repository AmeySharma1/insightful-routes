...
async function consumeOtp(email:string,code:string,purpose:OtpPurpose){const normalized=normalize(email),otp=await getOtp(normalized,purpose);if(!otp||otp.expiresAt<Date.now()||otp.attempts>=5)throw new Error("Invalid or expired code");otp.attempts+=1;if(hashCode(otp.id,code)!==otp.codeHash){await updateOtp(otp);throw new Error("Invalid or expired code");}await deleteOtp(normalized,purpose);return otp;}
