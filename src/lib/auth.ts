...
export async function authRequest<T>(path:string,body?:unknown,method="POST"){const init:RequestInit={method,credentials:"include",headers:{"content-type":"application/json",...(accessToken?{authorization:`Bearer ${accessToken}`}:{})}};if(body!==undefined)init.body=JSON.stringify(body);const response=await fetch(`${import.meta.env['VITE_API_URL']??"http://localhost:3000"}/api${path}`,init);const result=await response.json() as ApiEnvelope<T>;if(!response.ok||!result.success)throw new Error(result.error?.message??"Request failed");return result.data as T;}
